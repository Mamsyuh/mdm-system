...
<head>
    ...
    <style>
        ...
        .avatar-admin {
            background-color: #d97706; /* Warna Amber */
    ...
    <div class="flex">
        ...
        <main id="main-content" class="flex-1 p-4 md:p-6 transition-all duration-300">
            ...
            <div class="bg-gradient-to-r from-amber-800 to-red-800 rounded-2xl p-6 text-white mb-6 shadow-2xl">
                <h2 class="text-2xl font-bold">Halo, Administrator {{ auth()->user()->name }}!</h2>
                <p class="text-amber-200 mt-1">Dashboard Utama Pengelolaan Data. 📅 {{ now()->translatedFormat('l, d F Y') }}</p>
            </div>
            ...
            <div class="bg-white rounded-xl shadow-2xl p-4 mb-6">
                ...
                <div class="flex flex-wrap gap-4 items-center justify-start">
                    ...
                    <a href="{{ route('validasi.index') }}" class="flex items-center p-3 bg-yellow-100 rounded-lg hover:bg-yellow-200 transition duration-300 shadow-md text-sm font-semibold text-yellow-800">
                        <i class="fas fa-user-shield text-lg mr-2"></i> Cek Validasi ({{ $dataValid }})
                    </a>
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
                ...
                {{-- 1. Total Penduduk --}}
                <div class="stat-card bg-white rounded-xl p-5 shadow-lg border-l-4 border-amber-600 flex justify-between items-center">
                    <div>
                    ...
                    <div class="w-12 h-12 bg-amber-600 rounded-full flex items-center justify-center text-xl text-white shadow-md">
                        <i class="fas fa-users"></i>
                    </div>
                </div>
                
                {{-- 2. Data Valid --}}
                <div class="stat-card bg-white rounded-xl p-5 shadow-lg border-l-4 border-emerald-700 flex justify-between items-center">
                    <div>
                        <p class="text-3xl font-bold text-gray-800">{{ $dataValid ?? 0 }}</p>
                        <p class="text-gray-500 text-sm mt-1">Data Penduduk Valid</p>
                    </div>
                    <div class="w-12 h-12 bg-emerald-700 rounded-full flex items-center justify-center text-xl text-white shadow-md">
                        <i class="fas fa-check-circle"></i>
                    </div>
                </div>

                {{-- 3. Kepala Keluarga --}}
                <div class="stat-card bg-white rounded-xl p-5 shadow-lg border-l-4 border-blue-600 flex justify-between items-center">
                    <div>
                        <p class="text-3xl font-bold text-gray-800">{{ $totalKK ?? 0 }}</p>
                        <p class="text-gray-500 text-sm mt-1">Total Kepala Keluarga</p>
                    </div>
                    <div class="w-12 h-12 bg-blue-600 rounded-full flex items-center justify-center text-xl text-white shadow-md">
            ...
            <div class="bg-white rounded-xl shadow-2xl p-6 flex justify-between items-center border-l-4 border-red-700">
                <div>
                    ...
                    <p class="text-gray-600 text-sm mt-1">Ada **{{ $suratPending ?? 0 }}** surat yang menunggu aksi Anda.</p>
                </div>
                <a href="{{ route('surat.index') }}" class="bg-red-600 hover:bg-red-700 text-white font-bold py-2 px-4 rounded-lg shadow-md transition">
